...
def _key_chord_distribution(chord_pitch_out_of_key_prob):
    ...
    num_pitches_in_key = np.zeros([12, len(CHORDS)], dtype=np.int32)
    num_pitches_out_of_key = np.zeros([12, len(CHORDS)], dtype=np.int32)

    # For each key and chord, compute the number of chord notes in the key and the
    # number of chord notes outside the key.
    for key in range(12):
        key_pitches = set((key + offset) % 12 for offset in KEY_PITCHES)
        for i, chord in enumerate(CHORDS[1:]):
            root, kind = chord
            chord_pitches = set((root + offset) % 12
                                for offset in CHORD_KIND_PITCHES[kind])
...
def _key_chord_transition_distribution(
    ...
    mat = np.zeros([len(KEY_CHORDS), len(KEY_CHORDS)])

    for i, key_chord_1 in enumerate(KEY_CHORDS):
        key_1, chord_1 = key_chord_1
        chord_index_1 = i % len(CHORDS)

        for j, key_chord_2 in enumerate(KEY_CHORDS):
            key_2, chord_2 = key_chord_2
            chord_index_2 = j % len(CHORDS)
                ...
                if chord_1 != chord_2:
                    ...
                    mat[i, j] *= (
                        chord_change_prob * (
                            ...
                            key_chord_distribution[key_2, chord_index_1] / (len(CHORDS) -
                                                                            1)))
                else:
                    # No chord change.
...
def _key_chord_viterbi(chord_frame_loglik,
    ...
    for i, key_chord in enumerate(KEY_CHORDS):
    ...
    result = [(index // num_chords, CHORDS[index % num_chords])
            for index in path[::-1]]
...
def get_bar_number(sequence, bar_idx, bar_duration):
    start, stop = bar_idx * bar_duration, (bar_idx + 1) * bar_duration
    return [note for note in sequence if get_overlap((start, stop), (note.start, note.end)) > 0]




def get_sequence_with_chords(chords, sequence, bar_duration_in_ticks):
    sequence_with_chords = [
        (chord, get_bar_number(sequence, bar_idx, bar_duration_in_ticks)) for bar_idx, chord in enumerate(chords)
    ]
    return sequence_with_chords


def infer_chords_hierarchy(sequence,
    ...
    chords_2 = infer_chords_for_sequence(sequence,
                                         bar_duration_in_ticks // 2,
                                         max_chords * 2,
                                         **kwargs)

    chords_4 = infer_chords_for_sequence(sequence,
                                         bar_duration_in_ticks // 4,
                                         max_chords * 4,
                                         **kwargs)

    chords_hierarchy = [
        ([chord], chords_2[2 * bar_idx:2 * (bar_idx + 1)], chords_4[4 * bar_idx:4 * (bar_idx + 1)])
        for bar_idx, chord in enumerate(chords)
    ]

    return chords_hierarchy
...
def infer_chords_for_sequence(sequence,
    ...
    for frame, (key, chord) in enumerate(key_chords):
        ...
        # if PITCH_CLASS_NAMES[key] != current_key_name:
        ...
        #                 current_key_name, PITCH_CLASS_NAMES[key], time)
        #
        # current_key_name = PITCH_CLASS_NAMES[key]
        if chord == NO_CHORD:
            figure = NO_CHORD
        else:
            root, kind = chord
