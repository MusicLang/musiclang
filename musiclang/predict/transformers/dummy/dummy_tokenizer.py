...
TOKENIZER_VALUE = {val[1]: idx for idx, val in enumerate(TOKENS)}
...
def get_candidates_idx(text, authorized_tokens=None, can_terminate=False):
    ...
    if authorized_tokens is not None:
        possible_tokens_vec[:] = -float('inf')
        authorized_tokens += ['+', ',', ';', '(', ')', '%', '=', '.']
        authorized_tokens = list(set(authorized_tokens))
        idxs = [TOKENIZER_VALUE[token] for token in authorized_tokens ]
        possible_tokens_vec[idxs] = 0
    ...
    candidates_vec = np.zeros(len(TOKENIZER), dtype=float)
    idxs = np.asarray(sum([TOKENS_IDXS[c] for c in candidates], []))
    # Transform it to potential tokens
    candidates_vec[candidates_vec == 0] = - float('inf')
    candidates_vec[idxs] = 0.0
    candidates_vec[forbidden_instruments_idx] = - float('inf')
    if not can_terminate:
        candidates_vec[TERMINAL_IDX] = - float('inf')
    candidates_vec = candidates_vec + possible_tokens_vec
